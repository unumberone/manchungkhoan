...
/** Modal xác nhận đặt lệnh (portal) */
function ConfirmOrderModal({ open, onClose, side, data }) {
  React.useEffect(() => {
    if (!open) return;
  ...
  const onConfirm = () =>{
  alert('Xác nhận đặt lệnh thành công!');
  return onClose();
}
        ...
        <div className="modal__overlay" onClick={onClose}>
          ...
          >
            ...
            <div className="info-grid">
              <div className="item">
                <span className="label">Loại lệnh đặt</span>
                <span className={`value ${side === 'Long' ? 'txt-long' : 'txt-short'}`}>{side}</span>
              </div>
              <div className="item">
                <span className="label">Mã chứng khoán</span>
                <span className="value">{data.symbol}</span>
              </div>
              <div className="item">
                <span className="label">Loại lệnh</span>
                <span className="value">{data.orderType}</span>
              </div>
              <div className="item">
                <span className="label">Số tài khoản</span>
                <span className="value">{data.accountNo}</span>
              </div>
              <div className="item">
              ...
              <div className="item">
                <span className="label">Giá đặt</span>
            ...
            <div className="actions">
                  ...
                  <button 
                  type="submit" className="btn primary" onClick={onConfirm}>
                    Xác nhận
                  </button>
                </div>
          </div>
...
const Main = () => {
  ...
  const [symbol, setSymbol] = useState('VN30F2501');
  ...
  const handleOrderPlacement = (side) => {
    setModalSide(side);
    setModalOpen(true);
  };

  const handleConfirmOrder = () => {
    // TODO: gọi API đặt lệnh thực tế
    console.log('Xác nhận đặt lệnh:', {
      side: modalSide,
      symbol,
      orderType: orderStyle,
      accountNo: '6886886888',
      quantity: volume,
              ...
              <div className="asset-tabs">
                {tab2.map((menu, idx) => (
                  <div
                    ...
                    className={`asset-tab${location.pathname === menu.path ? ' active' : ''}`}
                  >
                    {menu.label}
                  </div>
                ))}
          ...
          <div className="trading-form">
            ...
            <div className="form-content">
              <div className="form-group">
                ...
                >
                  <option value="VN30F2501">VN30F2501</option>
              ...
              <div className="form-action">
                <button className="long-btn" onClick={() => handleOrderPlacement('Long')}>
                  Long
                </button>
                <button className="short-btn" onClick={() => handleOrderPlacement('Short')}>
                  Short
                </button>
              </div>
            </div>
